...
public class PresetNoiseRouterData {
    ...
    public static void bootstrap(Preset preset, BootstrapContext<DensityFunction> ctx) {
        ...
        ctx.register(NoiseRouterData.CONTINENTS, NTDensityFunctions.cell(CellSampler.Field.CONTINENT));
        ctx.register(NoiseRouterData.EROSION, NTDensityFunctions.cell(CellSampler.Field.EROSION));
        ctx.register(NoiseRouterData.RIDGES, NTDensityFunctions.cell(CellSampler.Field.WEIRDNESS));

        DensityFunction height = NoiseRouterData.registerAndWrap(ctx, HEIGHT, NTDensityFunctions.cell(CellSampler.Field.HEIGHT));
        DensityFunction offset = NoiseRouterData.registerAndWrap(ctx, NoiseRouterData.OFFSET, DensityFunctions.add(DensityFunctions.constant(NoiseRouterData.GLOBAL_OFFSET - 0.5F), DensityFunctions.mul(NTDensityFunctions.clampToNearestUnit(height, properties.worldHeight), DensityFunctions.constant(2.0D))));
    ...
    protected static NoiseRouter overworld(Preset preset, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParams, HolderGetter<Noise> noises) {
        ...
        DensityFunction temperature = NTDensityFunctions.cell(CellSampler.Field.TEMPERATURE);
        DensityFunction vegetation = NTDensityFunctions.cell(CellSampler.Field.MOISTURE);
...
//        DensityFunction finalDensity = DensityFunctions.min(NoiseRouterData.postProcess(slideOverworld(slopedCheeseCaves, -worldDepth)), NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.NOODLE));
    ...
    private static ResourceKey<DensityFunction> createKey(String string) {
        return ResourceKey.create(Registries.DENSITY_FUNCTION, NTCommon.location(string));
    }
}