...
import neoterra.data.worldgen.preset.settings.StructureSettings;
import neoterra.data.worldgen.preset.settings.StructureSettings.StructureSetEntry;
...
public class StructureSettingsPage extends PresetEditorPage {
	...
	public Component title() {
		return Component.translatable(NTTranslationKeys.GUI_STRUCTURE_SETTINGS_TITLE);
	}
	...
	public void init() {
		...
		structures.entries.forEach((key, entry) -> {
			...
			Slider spacing = PresetWidgets.createIntSlider(entry.spacing, 0, 1000, NTTranslationKeys.GUI_SLIDER_SPACING, (slider, value) -> {
			...
			Slider separation = PresetWidgets.createIntSlider(entry.separation, 0, 1000, NTTranslationKeys.GUI_SLIDER_SEPARATION, (slider, value) -> {
			...
			ValueButton<Integer> salt = PresetWidgets.createRandomButton(NTTranslationKeys.GUI_BUTTON_SALT, entry.salt, (value) -> {
				entry.salt = value;
			});
			CycleButton<Boolean> disabled = PresetWidgets.createToggle(entry.disabled, NTTranslationKeys.GUI_BUTTON_DISABLED, (button, value) -> {
