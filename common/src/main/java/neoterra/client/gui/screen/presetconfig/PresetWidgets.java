...
import neoterra.client.gui.widget.Slider.Format;
...
final class PresetWidgets {
	...
	public static CycleButton<Boolean> createToggle(boolean initial, String text, CycleButton.OnValueChange<Boolean> callback) {
		CycleButton<Boolean> button = CycleButton.booleanBuilder(Component.translatable(NTTranslationKeys.GUI_BUTTON_TRUE), Component.translatable(NTTranslationKeys.GUI_BUTTON_FALSE)).withInitialValue(initial).create(-1, -1, -1, -1, Component.translatable(text), callback);
