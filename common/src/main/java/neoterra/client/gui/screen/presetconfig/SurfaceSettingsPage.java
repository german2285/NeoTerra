...
public class SurfaceSettingsPage extends PresetEditorPage {
	...
	public Component title() {
		return Component.translatable(NTTranslationKeys.GUI_SURFACE_SETTINGS_TITLE);
	}
	...
	public void init() {
		...
		this.rockVariance = PresetWidgets.createIntSlider(erosion.rockVariance, 0, 256, NTTranslationKeys.GUI_SLIDER_ROCK_VARIANCE, (slider, value) -> {
			erosion.rockVariance = (int) slider.scaleValue(value);
			return value;
		});
		this.rockMin = PresetWidgets.createIntSlider(erosion.rockMin, -1024, 1024, NTTranslationKeys.GUI_SLIDER_ROCK_MIN, (slider, value) -> {
			erosion.rockMin = (int) slider.scaleValue(value);
			return value;
		});
		this.dirtVariance = PresetWidgets.createIntSlider(erosion.dirtVariance, 0, 256, NTTranslationKeys.GUI_SLIDER_DIRT_VARIANCE, (slider, value) -> {
			erosion.dirtVariance = (int) slider.scaleValue(value);
			return value;
		});
		this.dirtMin = PresetWidgets.createIntSlider(erosion.dirtMin, -1024, 1024, NTTranslationKeys.GUI_SLIDER_DIRT_MIN, (slider, value) -> {
			erosion.dirtMin = (int) slider.scaleValue(value);
			return value;
		});
		this.rockSteepness = PresetWidgets.createFloatSlider(erosion.rockSteepness, 0.0F, 3.0F, NTTranslationKeys.GUI_SLIDER_ROCK_STEEPNESS, (slider, value) -> {
			erosion.rockSteepness = (float) slider.scaleValue(value);
			return value;
		});
		this.dirtSteepness = PresetWidgets.createFloatSlider(erosion.dirtSteepness, 0.0F, 3.0F, NTTranslationKeys.GUI_SLIDER_DIRT_STEEPNESS, (slider, value) -> {
			erosion.dirtSteepness = (float) slider.scaleValue(value);
			return value;
		});
		this.screeSteepness = PresetWidgets.createFloatSlider(erosion.screeSteepness, 0.0F, 3.0F, NTTranslationKeys.GUI_SLIDER_SCREE_STEEPNESS, (slider, value) -> {
			erosion.screeSteepness = (float) slider.scaleValue(value);
			return value;
		});
		
		this.left.addWidget(PresetWidgets.createLabel(NTTranslationKeys.GUI_LABEL_SURFACE_EROSION));
