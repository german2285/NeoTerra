...
import neoterra.data.worldgen.preset.settings.ContinentType;
...
public class WorldSettingsPage extends PresetEditorPage {
	...
	public Component title() {
		return Component.translatable(NTTranslationKeys.GUI_WORLD_SETTINGS_TITLE);
	}
	...
	public void init() {
		...
		this.continentType = PresetWidgets.createCycle(
			...
			continent.continentType, NTTranslationKeys.GUI_BUTTON_CONTINENT_TYPE, 
			(button, value) -> {
		...
		this.continentShape = PresetWidgets.createCycle(DistanceFunction.values(), continent.continentShape, NTTranslationKeys.GUI_BUTTON_CONTINENT_SHAPE, (button, value) -> {
			continent.continentShape = value;
			this.regenerate();
		});
		this.continentScale = PresetWidgets.createIntSlider(continent.continentScale, 100, 10000, NTTranslationKeys.GUI_SLIDER_CONTINENT_SCALE, (slider, value) -> {
			continent.continentScale = (int) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentJitter = PresetWidgets.createFloatSlider(continent.continentJitter, 0.5F, 1.0F, NTTranslationKeys.GUI_SLIDER_CONTINENT_JITTER, (slider, value) -> {
			continent.continentJitter = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentSkipping = PresetWidgets.createFloatSlider(continent.continentSkipping, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_CONTINENT_SKIPPING, (slider, value) -> {
			continent.continentSkipping = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentSizeVariance = PresetWidgets.createFloatSlider(continent.continentSizeVariance, 0.0F, 0.75F, NTTranslationKeys.GUI_SLIDER_CONTINENT_SIZE_VARIANCE, (slider, value) -> {
			continent.continentSizeVariance = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentNoiseOctaves = PresetWidgets.createIntSlider(continent.continentNoiseOctaves, 1, 5, NTTranslationKeys.GUI_SLIDER_CONTINENT_NOISE_OCTAVES, (slider, value) -> {
			continent.continentNoiseOctaves = (int) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentNoiseGain = PresetWidgets.createFloatSlider(continent.continentNoiseGain, 0.0F, 0.5F, NTTranslationKeys.GUI_SLIDER_CONTINENT_NOISE_GAIN, (slider, value) -> {
			continent.continentNoiseGain = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.continentNoiseLacunarity = PresetWidgets.createFloatSlider(continent.continentNoiseLacunarity, 1.0F, 10.0F, NTTranslationKeys.GUI_SLIDER_CONTINENT_NOISE_LACUNARITY, (slider, value) -> {
		...
		this.islandInland = PresetWidgets.createFloatSlider(controlPoints.islandInland, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_ISLAND_INLAND, (slider, value) -> {
			value = Math.min(value, this.islandCoast.getValue());
			controlPoints.islandInland = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.islandCoast = PresetWidgets.createFloatSlider(controlPoints.islandCoast, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_ISLAND_COAST, (slider, value) -> {
			value = Math.min(value, this.deepOcean.getValue());
			controlPoints.islandCoast = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.deepOcean = PresetWidgets.createFloatSlider(controlPoints.deepOcean, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_DEEP_OCEAN, (slider, value) -> {
		...
		this.shallowOcean = PresetWidgets.createFloatSlider(controlPoints.shallowOcean, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_SHALLOW_OCEAN, (slider, value) -> {
		...
		this.beach = PresetWidgets.createFloatSlider(controlPoints.beach, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_BEACH, (slider, value) -> {
			value = Mth.clamp(value, this.shallowOcean.getValue(), this.coast.getValue());
			controlPoints.beach = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.coast = PresetWidgets.createFloatSlider(controlPoints.coast, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_COAST, (slider, value) -> {
			value = Mth.clamp(value, this.beach.getValue(), this.inland.getValue());
			controlPoints.coast = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.inland = PresetWidgets.createFloatSlider(controlPoints.inland, 0.0F, 1.0F, NTTranslationKeys.GUI_SLIDER_INLAND, (slider, value) -> {
			value = Math.max(value, this.coast.getValue());
			controlPoints.inland = (float) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.spawnType = PresetWidgets.createCycle(SpawnType.values(), properties.spawnType, NTTranslationKeys.GUI_BUTTON_SPAWN_TYPE, (button, value) -> {
			properties.spawnType = value;
			this.regenerate();
		});
		this.worldHeight = PresetWidgets.createIntSlider(properties.worldHeight, 0, 1024, NTTranslationKeys.GUI_SLIDER_WORLD_HEIGHT, (slider, value) -> {
		...
		this.worldDepth = PresetWidgets.createIntSlider(properties.worldDepth, 0, 1024, NTTranslationKeys.GUI_SLIDER_WORLD_DEPTH, (slider, value) -> {
			int nearestMultiple = getNearestMultiple(slider, (float) value, 16);
			properties.worldDepth = nearestMultiple;
			return slider.getSliderValue(nearestMultiple);
		});
		this.seaLevel = PresetWidgets.createIntSlider(properties.seaLevel, 0, 255, NTTranslationKeys.GUI_SLIDER_SEA_LEVEL, (slider, value) -> {
			properties.seaLevel = (int) slider.scaleValue(value);
			this.regenerate();
			return value;
		});
		this.lavaLevel = PresetWidgets.createIntSlider(properties.lavaLevel, -1024, 128, NTTranslationKeys.GUI_SLIDER_LAVA_LEVEL, (slider, value) -> {
			properties.lavaLevel = (int) slider.scaleValue(value);
			return value;
		});
		
		this.left.addWidget(PresetWidgets.createLabel(NTTranslationKeys.GUI_LABEL_CONTINENT));
		...
		this.left.addWidget(PresetWidgets.createLabel(NTTranslationKeys.GUI_LABEL_CONTROL_POINTS));
		...
		this.left.addWidget(PresetWidgets.createLabel(NTTranslationKeys.GUI_LABEL_PROPERTIES));
