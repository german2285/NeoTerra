...
class MixinRandomState {
	...
	public void neoterra$NTRandomState$initialize(RegistryAccess registries) {
		NTCommon.debug("NTRandomState.initialize: seed={}, hasContext={}", this.seed, this.hasContext);
		...
		functions.get(NTDensityFunctionTags.ADDITIONAL_NOISE_ROUTER_FUNCTIONS).ifPresent((set) -> {
			NTCommon.debug("Mapping {} additional noise router functions through density wrapper", set.size());
		...
		presets.get(Preset.KEY).ifPresentOrElse((presetHolder) -> {
			...
			NTCommon.debug("Preset {} loaded into RandomState", Preset.KEY.location());

			if(this.hasContext) {
				PerformanceConfig config = PerformanceConfig.read(PerformanceConfig.DEFAULT_FILE_PATH)
					.resultOrPartial(NTCommon.LOGGER::error)
					.orElseGet(PerformanceConfig::makeDefault);
				NTCommon.debug("Building GeneratorContext: tileSize={}, batchCount={}, parallel={}",
			...
			if(this.hasContext) {
				NTCommon.debug("RandomState requires preset but registry has no entry for {}", Preset.KEY.location());
