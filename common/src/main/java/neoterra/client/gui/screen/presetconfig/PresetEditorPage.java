...
public abstract class PresetEditorPage extends BisectedPage<PresetConfigScreen, AbstractWidget, AbstractWidget> {
	...
	public void init() {
		...
		this.zoom = PresetWidgets.createIntSlider(Optional.ofNullable(this.zoom).map(Slider::getLerpedValue).orElse(68.0D).intValue(), 1, 100, NTTranslationKeys.GUI_SLIDER_ZOOM, (slider, value) -> {
			this.regenerate();
			return value;
		});
		...
		this.seed = PresetWidgets.createRandomButton(NTTranslationKeys.GUI_BUTTON_SEED, (int) this.screen.getSettings().options().seed(), (i) -> {
	...
	public class Preview extends Button {
	    ...
	    private ResourceLocation textureId = Minecraft.getInstance().getTextureManager().register(NTCommon.MOD_ID + "-preview-framebuffer", this.texture); 
	    private Tile tile;
	    ...
	    private Component[] legendLabels = { Component.translatable(NTTranslationKeys.GUI_LABEL_PREVIEW_AREA), Component.translatable(NTTranslationKeys.GUI_LABEL_PREVIEW_TERRAIN), Component.translatable(NTTranslationKeys.GUI_LABEL_PREVIEW_BIOME) };
